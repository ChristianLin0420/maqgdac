...
def init_dir(env, env_args, algo, entity_name, exp_name, seed, logger_path):
    ...
    # Create a cleaned config dictionary for wandb
    # Convert non-serializable parts of the config
    def clean_for_wandb(config_dict):
        cleaned = {}
        for k, v in config_dict.items():
            if isinstance(v, dict):
                cleaned[k] = clean_for_wandb(v)
    ...
    wandb_config = {
        ...
        "exp_name": exp_name,
        "seed": seed,
        "env_args": clean_for_wandb(env_args),
    }
    ...
    wandb_run = wandb.init(
        ...
        config=wandb_config,
        dir=log_path,
        sync_tensorboard=True,  # Sync TensorBoard events
    )
    
    models_path = os.path.join(results_path, "models")
    os.makedirs(models_path, exist_ok=True)
    return results_path, log_path, models_path, writter, wandb_run
...
def save_config(args, algo_args, env_args, run_dir, wandb_run=None):
    ...
    # Also update wandb config with algo_args if wandb_run is provided
    if wandb_run is not None:
        # Clean algorithm args for wandb
        def clean_for_wandb(config_dict):
            cleaned = {}
            for k, v in config_dict.items():
                if isinstance(v, dict):
        ...
        # Update wandb config with algorithm args
        wandb_run.config.update({"algo_args": clean_for_wandb(algo_args)}, allow_val_change=True)
