...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if not self.algo_args["render"]["use_render"]:  # train, not render
            self.run_dir, self.log_dir, self.save_dir, self.writter, self.wandb_run = init_dir(
                args["env"],
                env_args,
                args["algo"],
                args["entity_name"],
        ...
        if self.algo_args["render"]["use_render"] is False:  # train, not render
            ...
            self.logger = LOGGER_REGISTRY[args["env"]](
                args, algo_args, env_args, self.num_agents, self.writter, self.wandb_run, self.run_dir
            )
    ...
    def close(self):
        ...
        self.envs.close()
        if self.algo_args["eval"]["use_eval"] and not self.algo_args["render"]["use_render"]:
            self.eval_envs.close()
        if not self.algo_args["render"]["use_render"]:
            self.logger.close()
            self.writter.close()
            # Already closed in logger.close() which calls wandb_run.finish()
